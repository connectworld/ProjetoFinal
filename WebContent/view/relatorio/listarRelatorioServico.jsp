...
<c:import url="../menu.jsp"></c:import>
	...
	<div align="center"><h3 class="h3">Serviços</h3></div>
		<table id="tableProduto" class="table">
			...
			<c:forEach var="pedidoServico" items="${listaPedidoServico}" varStatus="id">
					<tr bgcolor="#${id.count % 2 == 0 ? 'aaee88' : 'blue' }">
						<td>${pedidoServico.cod }</td>
						<td>${pedidoServico.cliente.nome}</td>
						<td>${pedidoServico.situacao}</td>
						<td>
							<fmt:formatDate value="${pedidoServico.data}" pattern="dd/MM/yyyy" />
						</td>
						<td>${pedidoServico.valor}</td>
						
						<td>${pedidoServico.codigo.nome}</td>
					</tr>
